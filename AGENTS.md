...
- Motion primitives live in `src/utils/motion.ts` (`fadeOutThenHide`, `flipFromRect`, `revealIn`, `collapse`); `prefersReducedMotion()` must be honored. It reads **only** the site's "reduce motion" display setting (`html.motion-reduced`, seeded before first paint by the inline boot script in `src/layouts/Layout.astro` from `localStorage["mc-motion"]`, and written by `DisplaySettings.svelte`). The theme deliberately does **not** consult the OS `prefers-reduced-motion` preference (project decision, 2026-09-16): the site switch is the single source of truth, so no CSS media query and no JS branch may gate motion on the system preference.
